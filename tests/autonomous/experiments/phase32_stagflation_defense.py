"""Phase 32: stagflation-specific defensive universe.

When classifier predicts LG_HI (stagflation) with high confidence, use a
commodity/inflation-hedge universe: XLE, GLD, DBC, TLT, SHY.
"""
...
TRANS_DEF = STABLE + ["TLT","AGG","XLV"]
# Stagflation index in REGIME_CLASSES — find it
_STAG_IDX = None
try:
    _STAG_IDX = [i for i,c in enumerate(REGIME_CLASSES) if "stagflation" in c or "lg_hi" in c][0]
except Exception:
    _STAG_IDX = 3
...
def run_stagflation_def(stable_uni, trans_def_uni, stag_uni, sig, tier=0.50, stag_thr=0.40):
    ...
    eng_s = mk_eng(stable_uni); eng_t = mk_eng(trans_def_uni); eng_stag = mk_eng(stag_uni)
    ...
    for rd, pos in month_last.items():
        ...
        is_stag_pred = (pr[i] == _STAG_IDX) and (not np.isnan(mp[i])) and (mp[i] >= stag_thr)
        is_trans = (pr[i]!=cur[i]) and (pr[i]>=0) and (not np.isnan(mp[i])) and (mp[i]>=tier)
        if is_stag_pred:
            eng = eng_stag
        elif is_trans:
            eng = eng_t
...
def main():
    print("stagflation class idx:", _STAG_IDX, "classes:", REGIME_CLASSES)
    sig = rankagg([(42,1),(63,3),(126,1)])
    log_path = HERE.parent / "LOG.md"
    ts = dt.datetime.now().isoformat(timespec="seconds")
    lines = [f"\n### phase32_stagflation_defense  {ts}\n",
             "| name | CAGR | Sharpe | MaxDD | verdict |","|---|---|---|---|---|"]
    experiments = [
        ("P32.01_stag_XLE_GLD_DBC_TLT_SHY",  ["XLE","GLD","DBC","TLT","SHY"], 0.40),
        ("P32.02_stag_XLE_GLD_TLT_SHY",      ["XLE","GLD","TLT","SHY"], 0.40),
        ("P32.03_stag_GLD_TLT_DBC_SHY",      ["GLD","TLT","DBC","SHY"], 0.40),
        ("P32.04_stag_GLD_DBC_SHY",          ["GLD","DBC","SHY"], 0.40),
        ("P32.05_stag_hard_SHY_only",        ["SHY"], 0.40),
        ("P32.06_stag_hi_conf_60",           ["XLE","GLD","DBC","TLT","SHY"], 0.60),
    ]
    for name, stag_uni, stag_thr in experiments:
        try: s = run_stagflation_def(STABLE, TRANS_DEF, stag_uni, sig, stag_thr=stag_thr)
