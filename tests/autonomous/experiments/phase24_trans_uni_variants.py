"""Phase 24: further transition universe variants around the TLT finding."""
...
def run_rc(stable_uni, trans_uni, sig, gated_p):
    ...
    def mk_eng(uni):
        cfg = copy.deepcopy(_STATE["cfg"]); cfg.update({"universe":uni,"top1_weight":0.62,"top3_weight":0.38})
...
def main():
    ...
    lines = [f"\n### phase24_trans_uni_variants  {ts}\n",
             "| name | CAGR | Sharpe | MaxDD | verdict |","|---|---|---|---|---|"]

    experiments = [
        ("P24.01_trans_+TLT_+AGG",       STABLE, STABLE + ["TLT","AGG"]),
        ("P24.02_trans_+TLT_+XLV",       STABLE, STABLE + ["TLT","XLV"]),
        ("P24.03_trans_+TLT_+XLI",       STABLE, STABLE + ["TLT","XLI"]),
        ("P24.04_trans_+AGG",            STABLE, STABLE + ["AGG"]),
        ("P24.05_trans_+TLT_+EEM",       STABLE, STABLE + ["TLT","EEM"]),
        ("P24.06_trans_+TLT_+XLF",       STABLE, STABLE + ["TLT","XLF"]),
        ("P24.07_trans_only_tech_TLT_GLD", STABLE, ["SOXX","QQQ","IGV","TLT","GLD","SHY"]),
        ("P24.08_trans_no_xle_+TLT",     STABLE, ["SOXX","QQQ","IGV","GLD","SHY","TLT"]),
        ("P24.09_trans_full_+TLT_+DBC",  STABLE, STABLE + ["TLT","DBC"]),
        ("P24.10_trans_only_GLD_TLT_SHY",STABLE, ["GLD","TLT","SHY"]),
        # Also: try stable+TLT (not just transition)
        ("P24.11_stable_+TLT",           STABLE + ["TLT"], STABLE + ["TLT"]),
    ]
