"""Phase 35: alternative vol proxy for inv-vol weighting.

Current: raw ATR (dollars). This structurally lets cheaper ETFs (XLE) dominate.
Alternative: normalized ATR% (ATR / price) — equalizes across price levels.
Also: 63d realized vol instead of ATR.
"""
...
def load_prices(tickers):
    ...
    for t in tickers:
        ...
        if p.exists():
            out[t] = pd.read_parquet(p)["adj_close"].reindex(b.dates).ffill()
    return pd.DataFrame(out)


def run_custom_vol(sig, vol_proxy_arr):
    """vol_proxy_arr: ndarray of shape (n_days, n_tickers_in_atr_cols_order)."""
    ...
    def mk_eng(uni):
        ...
        eng = PortfolioEngine(shim, gated, cfg)
        # Replace atr_arr post-construction using the new proxy
        from strategy.position_sizing import build_atr_array
        new_atr_df = pd.DataFrame(vol_proxy_arr, index=b.dates, columns=list(b.atr21.columns))
        eng.atr_arr = build_atr_array(new_atr_df, eng.universe, eng.dates)
        return eng
...
def main():
    ...
    _load(); b = _STATE["bundle"]
    px = load_prices(list(b.atr21.columns))
    px = px.reindex(b.dates).ffill()
    px = px[list(b.atr21.columns)]  # order
    # Percent ATR = atr21 / price
    atr_pct = b.atr21.values / np.where(px.values>0, px.values, np.nan)
    # 63d realized vol
    ret = px.pct_change()
    vol63 = ret.rolling(63).std().values * np.sqrt(252)

    log_path = HERE.parent / "LOG.md"
    ts = dt.datetime.now().isoformat(timespec="seconds")
    lines = [f"\n### phase35_vol_proxy  {ts}\n",
             "| name | CAGR | Sharpe | MaxDD | verdict |","|---|---|---|---|---|"]
    for name, arr in [("P35.01_atr_pct", atr_pct), ("P35.02_vol63", vol63)]:
        try: s = run_custom_vol(sig, arr)
