"""Phase 28: fine grid around the tier_proba=0.50 champion — split and transition universe variants."""
...
def run_tier(stable_uni, trans_uni, sig, split_s=(0.62,0.38), split_t=None, uni_tier=0.50, sig_gate=0.40):
    ...
    gated[w&lc]=cur[w&lc]
    if split_t is None: split_t = split_s
    ...
    eng_s = mk_eng(stable_uni, split_s); eng_t = mk_eng(trans_uni, split_t)
...
def main():
    ...
    lines = [f"\n### phase28_tier_proba_grid  {ts}\n",
             "| name | CAGR | Sharpe | MaxDD | verdict |","|---|---|---|---|---|"]

    TRANS_SET = {
        "TLT_AGG_XLV": STABLE + ["TLT","AGG","XLV"],
        "TLT_AGG_XLV_XLF": STABLE + ["TLT","AGG","XLV","XLF"],
        "TLT_AGG_XLV_XLI": STABLE + ["TLT","AGG","XLV","XLI"],
        "TLT_AGG_XLV_DBC": STABLE + ["TLT","AGG","XLV","DBC"],
        "TLT_AGG_XLV_IWM": STABLE + ["TLT","AGG","XLV","IWM"],
        "TLT_AGG_XLV_EEM": STABLE + ["TLT","AGG","XLV","EEM"],
        "TLT_XLV_only":  STABLE + ["TLT","XLV"],
        "AGG_XLV_only":  STABLE + ["AGG","XLV"],
    }
    experiments = []
    for name, tu in TRANS_SET.items():
        for t in (0.45, 0.50, 0.55):
            experiments.append((f"P28.01_{name}_t{int(t*100)}", tu, t, None))
    # Different split in transition (more defensive)
    for w1 in (0.40, 0.45, 0.50, 0.55):
        experiments.append((f"P28.02_transsplit_{int(w1*100)}", STABLE + ["TLT","AGG","XLV"], 0.50, (w1, 1-w1)))

    for name, tu, tier, split_t in experiments:
        try: s = run_tier(STABLE, tu, sig, uni_tier=tier, split_t=split_t)
