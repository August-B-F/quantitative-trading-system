"""Phase 45: leave-one-out ablation on P44's 8 extra classifier features.

Validates whether each individual extra feature matters or if the result is an
accidental interaction. Also tries: core + each single extra, core + each pair.
"""
...
def main():
    ...
    lines = [f"\n### phase45_loo_ablation  {ts}\n",
             "| name | removed | CAGR | Sharpe | MaxDD | acc | verdict |",
             "|---|---|---|---|---|---|---|"]

    # Full
    pred, mp, acc = wf_retrain(cp + EXTRAS)
    s = run_full(pred, mp, sig); p,_ = haircut_verdict(s)
    print("full", fmt(s), f"acc={acc:.3f}")
    lines.append(f"| full_8 | none | {s['cagr']*100:.2f}% | {s['sharpe']:.2f} | {s['max_dd']*100:.2f}% | {acc:.3f} | PASS |")

    # LOO
    for i, removed in enumerate(EXTRAS):
        feats = cp + [e for e in EXTRAS if e != removed]
        pred, mp, acc = wf_retrain(feats)
        s = run_full(pred, mp, sig); p,_ = haircut_verdict(s)
        short = removed.split("__")[-1]
        print(f"LOO -{short}", fmt(s), f"acc={acc:.3f}")
        lines.append(f"| loo_{i} | {short} | {s['cagr']*100:.2f}% | {s['sharpe']:.2f} | {s['max_dd']*100:.2f}% | {acc:.3f} | {'PASS' if p else 'fail'} |")
