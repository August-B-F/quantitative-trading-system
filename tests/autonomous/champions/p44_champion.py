"""CHAMPION as of 2026-04-13 16:45 local.

Extends P42 by retraining the walk-forward classifier with 8 additional
credit/yield-curve features beyond the canonical CORE-50 set. The extra
features improve walk-forward 4-class accuracy from 0.665 to 0.679.
...
Full walk-forward stats:
    CAGR   = 28.34%  (baseline 23.61%, +4.73pp)
    Sharpe =  1.78   (baseline 1.50,  +0.28)
    MaxDD  = -12.66% (baseline -12.94%)

Post-50%-haircut: CAGR ~25.98%, Sharpe ~1.64.

Sub-period:
  2010-15: 20.96/1.65/-7.88 (+7.22pp CAGR over baseline)
  2016-20: 25.08/1.47/-12.66 (+4.54pp)
  2021-26: 40.14/2.25/-5.91 (+1.77pp)

⚠ Caveat: the 8-feature selection came from a keyword-filter over a 12-feature grid.
Subject to selection bias. Use the reference `p42_champion.py` as a conservative
fallback if this reproduction fails.

Requires `cache/pred_proba_p44_05.pkl` — built by running this file once, or by
running the retraining snippet in experiments/phase44_crossasset_features.py.
"""
...
def run_champion():
    _load()
    b = _STATE["bundle"]
    cache = HERE.parent / "cache" / "pred_proba_p44_05.pkl"
    if cache.exists():
...
if __name__ == "__main__":
    s = run_champion()
    print("CHAMPION P44 (classifier + credit + yc features)")
    print(fmt(s))
